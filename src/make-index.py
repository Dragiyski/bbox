...
def main():
    ...
    print('Reading: %r...' % args.file, file=sys.stderr)
    mesh = Mesh.from_wavefront_stream(args.file)
    print('Mesh with:', file=sys.stderr)
    print(f' - Vertices: {len(mesh.vertices)}', file=sys.stderr)
    print(f' - Normals: {len(mesh.normal)}', file=sys.stderr)
    print(f' - TexCoords: {len(mesh.texcoord)}', file=sys.stderr)
    print(f' - Triangles: {len(mesh.face)}', file=sys.stderr)
